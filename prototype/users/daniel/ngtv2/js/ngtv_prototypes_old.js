...
function moreWaysHandler(el){
	...
	if(el.hasClass('selected')){
		...
		$('#side_module').stop().fadeOut(250,function(){
			$('#single_stack').stop().animate({width: $('#single_stack').width()-220},250);
...
function roloForwardCard(number){
	...
	if(number > $('.single_card').length){
		...
		var selectedCard = $('.single_card:nth-child('+count+')');
	}
...
/* 1. sub_nav_animation script */
	
	if(document.title == 'NGTV: Sub Navigation Animation'){

		$(window).load(function(){
			var marginSpacing = ($(window).height() - $('#single_stack').height()) / 2;
				$('#single_stack')
					.width($('#single_stack').width())
					.css({
						display: 'block',
		...
		// move into the HTML structures later on
...
/* 2. single_stack_animation script */

	}else if(document.title == 'NGTV: Single Stack Animation'){

		$(window).load(function(){
			subNavHandler(0,'singlestack',10);
		});
	       
			/* TOUCH CONTROLS - 03/15/15 */
