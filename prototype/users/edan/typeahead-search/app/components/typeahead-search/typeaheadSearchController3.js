...
(function() {
  ...
  app.controller("TypeaheadSearchController", ['$scope', '$rootScope', '$filter', '$interval', '$http', function($scope, $rootScope, $filter, $interval, $http) {
    ...
    $scope.$watch('searchText', function(val) {
      ...
      if ((val) && val.length > 0) {
        var url = 'https://edm.dtvce.com/SmartSearchWS/rest/search15?searchText=' + val + '&zipCode=26105&maxResults=3&pgmWithTMSid=true&output=json&includeottcontent=true&includeottpreview=false&callback=JSON_CALLBACK';
        $http.jsonp(url).
