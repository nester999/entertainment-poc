...
(function() {
  ...
  var detectDevice = {
    ...
    any: function() {
      return (detectDevice.Android() || detectDevice.BlackBerry() || detectDevice.iOS() || detectDevice.Opera() || detectDevice.Windows());
    }
  };
  ...
  app.controller("StackedViewController", function($scope, $http, $filter, $timeout, $document) {
    ...
    $scope.staggerAnimTime = 20;
    $scope.velocityMultiplier = 4;
    $scope.yDropRate = 2;
    $scope.zDropoffPoint = -114;
    ...
    $scope.scrollCardsDown = function(event) {
      ...
      angular.forEach(posters, function(poster, index) {
        ...
        var currentPosterStyle = poster.style,
          currentPosterTransform;


        if (detectDevice.any()) {
          currentPosterTransform = currentPosterStyle.webkitTransform;
          $scope.isMobile = true;
        } else {
          currentPosterTransform = currentPosterStyle.transform;
        }
        ...
        setTimeout(function() {
          ...
          if ($scope.isMobile) {
            if (currentTzDownValue > $scope.zDropoffPoint && currentTyValue === $scope.translateYTrack) {
              poster.style.webkitTransform = 'translateZ(' + (currentTzDownValue + (50 + 32 * ((posters.length - 1) - index)) * Math.abs($scope.velocityMultiplier * event.velocityY)) + 'px) translateY(' + (currentTyValue + $scope.yDropRate) * Math.abs($scope.velocityMultiplier * event.velocityY) + 'px)';
            } else if (currentTzDownValue >= $scope.zDropoffPoint) {
              // console.log('stepping over stack');
              poster.style.webkitTransform = 'translateZ(' + (200 - (index * $scope.slowZoneSpacer)) + 'px) translateY(' + (currentTyValue + $scope.yDropRate) * Math.abs($scope.velocityMultiplier * event.velocityY) + 'px)';
            } else if (currentTzDownValue || currentTyValue < 0) {
              poster.style.webkitTransform = 'translateZ(' + ($scope.frontOfStack) + 'px) translateY(' + -3000 + 'px)';

            } else {
              // console.log(poster.alt + ', poster[' + index + '].transform: ' + 'translateZ(' + newTzValue + 'px) translateY(' + $scope.translateYTrack + 'px)');
              poster.style.webkitTransform = 'translateZ(' + (currentTzDownValue + (50 + 32 * ((posters.length - 1) - index)) * Math.abs($scope.velocityMultiplier * event.velocityY)) + 'px) translateY(' + $scope.translateYTrack + 'px)';
            }
          } else {
            // card is over the z line, and should start heading downward
            if (currentTzDownValue >= $scope.zDropoffPoint && currentTyValue > $scope.translateYTrack && newTyValue < $scope.translateYTrack) {
              poster.style.transform = 'translateZ(' + (currentTzDownValue + (50 + 32 * ((posters.length - 1) - index)) * Math.abs($scope.velocityMultiplier * event.velocityY)) + 'px) translateY(' + (currentTyValue + $scope.yDropRate) * Math.abs($scope.velocityMultiplier * event.velocityY) + 'px)';
            } else if (currentTzDownValue >= $scope.zDropoffPoint) {
              // console.log('stepping over stack');
              poster.style.transform = 'translateZ(' + (200 - (index * $scope.slowZoneSpacer)) + 'px) translateY(' + (currentTyValue + $scope.yDropRate) * Math.abs($scope.velocityMultiplier * event.velocityY) + 'px)';
            } else if (currentTyValue < -100 || newTyValue < -200) {
              poster.style.transform ='translateZ(' + ($scope.frontOfStack) + 'px) translateY(' + -3000 + 'px)';

            } 
            // else if ((currentTzDownValue - Math.abs($scope.zDropoffPoint)) < 100 ) {
            //   poster.style.transform = 'translateZ(' + 0.5*(newTzValue + (50 + 32 * ((posters.length - 1) - index)) * Math.abs($scope.velocityMultiplier * event.velocityY)) + 'px) translateY(' + $scope.translateYTrack + 'px)';

            // }

            else {
              // console.log(poster.alt + ', poster[' + index + '].transform: ' + 'translateZ(' + newTzValue + 'px) translateY(' + $scope.translateYTrack + 'px)');
              poster.style.transform = 'translateZ(' + (newTzValue + (50 + 32 * ((posters.length - 1) - index)) * Math.abs($scope.velocityMultiplier * event.velocityY)) + 'px) translateY(' + $scope.translateYTrack + 'px)';
            }
          }



        }, ($scope.staggerAnimTime * index));
      });
    };
          ...
          if (!docked) {
            if ($scope.isMobile) {
              ...
              if (currentTzValue >= ($scope.backStop + (index * $scope.slowZoneSpacer)) && newTzValue > ($scope.backStop + index)) {
                ...
                poster.style.webkitTransform = 'translateZ(' + newTzValue + 'px) translateY(' + $scope.translateYTrack + 'px)';
              ...
              // cards that are popping back up into view
                ...
                poster.style.webkitTransform = 'translateZ(' + ($scope.backStop + (index * $scope.slowZoneSpacer)) + 'px) translateY(' + (currentTyValue - $scope.yDropRate) * Math.abs(event.velocityY) + 'px)';
              } else {
                // console.log(poster.alt + ', poster[' + index + '].transform: ' + 'translateZ(' + newTzValue + 'px) translateY(' + $scope.translateYTrack + 'px)');
                poster.style.webkitTransform = 'translateZ(' + ($scope.backStop + (index * $scope.slowZoneSpacer)) + 'px) translateY(' + $scope.translateYTrack + 'px)';
              }
            } else {
              ...
              // cards that are popping back up into view
                ...
                poster.style.transform = 'translateZ(' + ($scope.backStop + (index * $scope.slowZoneSpacer)) + 'px) translateY(' + (currentTyValue - $scope.yDropRate) * Math.abs(event.velocityY) + 'px)';
              } else {
                // console.log(poster.alt + ', poster[' + index + '].transform: ' + 'translateZ(' + newTzValue + 'px) translateY(' + $scope.translateYTrack + 'px)');
                poster.style.transform = 'translateZ(' + ($scope.backStop + (index * $scope.slowZoneSpacer)) + 'px) translateY(' + $scope.translateYTrack + 'px)';
              }
            }
          }
        }, ($scope.staggerAnimTime * index));
