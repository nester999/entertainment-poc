...
(function() {
  ...
  app.controller("StackedViewController", function($scope, $http, $filter, $timeout, $document) {
    ...
    $scope.scrollCardsDown = function(event) {
      // console.log("scrollCardsDown: ");
      $('#horizon').css('color', 'green');
      ...
      var firstPoster = $('.movie-poster img').first();
      ...
      angular.forEach(posters, function(poster, index) {
        ...
        console.log('scrollCardsUp Ty: ' + currentTyValue);
        console.log('event.deltaY: ' + event.deltaY);



        // console.log(event.deltaY);
        var newTzValue = parseInt(currentTzValue + event.deltaY);
        // console.log(newTzValue);
        console.log('translateZ(+=' + event.deltaY + 'px) translateY(' + currentTyValue + 'px)');
        console.log(poster.style.transform);
        poster.style.transform = 'translateZ(' + (newTzValue) + 'px) translateY(' + currentTyValue + 'px)';
        // console.log('translateZ(' + (currentTzValue + event.deltaY - zOffset) * 0.5 + 'px) translateY(' + currentTyValue + 'px)');
        // poster.style.transform = 'translateZ(' + (currentTzValue + event.deltaY - zOffset) * 0.3 + 'px) translateY(' + currentTyValue + 'px)';
        // poster.style.transform = 'translateZ(' + (getZAxisMotionPathValue((currentTyValue + event.deltaY) * decay) - zOffset) + 'px) translateY(' + (currentTyValue + event.deltaY) + 'px)';
      });
    };
    ...
    $scope.scrollCardsUp = function(event) {
      ...
      // var firstPoster = $('.movie-poster img').first();
      // var firstPosterStyle = firstPoster[0].style;
      // var firstPosterTransform = firstPosterStyle.transform;


      // // extract the translateZ pixel value from the current element's transform
      // var TzString = firstPosterTransform.split('(')[1].split(')')[0];
      // var currentTzValue = parseInt(TzString.substring(0, TzString.length - 2));
      // // console.log(currentTzValue);

      // // extract the translateX pixel value from the current element's transform
      // var TyString = firstPosterTransform.split('(')[2].split(')')[0];
      // var currentTyValue = parseInt(TyString.substring(0, TyString.length - 2));
      // // console.log(currentTyValue);
      ...
      angular.forEach(posters, function(poster, index) {
        ...
        var newTzValue = parseInt(currentTzValue + event.deltaY);
        // poster.style.transform = 'translateZ(' + (currentTzValue + event.deltaY) * 0.3 + 'px) translateY(' + currentTyValue + 'px)';
        console.log('poster[' + index + '].transform: ' + 'translateZ(' + newTzValue + 'px) translateY(' + currentTyValue + 'px)');
        poster.style.transform = 'translateZ(' + newTzValue + 'px) translateY(' + currentTyValue + 'px)';
        // poster.style.transform = 'translateZ(' + (getZAxisMotionPathValue((currentTyValue + event.deltaY) * decay) - zOffset) + 'px) translateY(' + (currentTyValue + event.deltaY) + 'px)';
      });
    };
    ...
    $scope.myCollapseCards = function(event) {
      if (!alreadyFiringEvent) {
        ...
        angular.forEach($scope.movieLinks, function(movieLink) {
          ...
          // }
          // else {
          // console.log(JS ON.stringify(movieLink.style.transform, null, 2));
