desktop.controller('desktop.ChannelModalController', function($scope, $element, $state, $stateParams, $previousState, $timeout, $dtvApi){
  //TO-DO: refactor

  //TO-DO: below hero carousel code is for POC only
  $dtvApi.details($stateParams.media_type, $stateParams.id).then(function(data){
    $scope.card = data;
    $scope.channelPath = 'https://www.directv.com/images/logos/channels/dark/large/830.png';
    $scope.channelName = 'HBO East HD';
    $scope.channelNumber = 501;
    $scope.channelPackage = 'Premium';
    $scope.mainChannelShow = 'Game of Thrones';
    $scope.hd = 'HD';
    $scope.channelFullName = 'Home Box Office';
    $scope.channelDescription = 'HBO (Home Box Office) is an American premium cable and satellite television network that is owned by Home Box Office Inc., an operating subsidiary ' +
                                'of Time Warner. HBO\'s programming consists primarily of theatrically released motion pictures and original television series, along with made-for-cable ' +
                                'movies and documentaries, boxing matches and occasional stand-up comedy.'; 
    $scope.mediaType = $stateParams.media_type;
    var channelGenres = [$scope.channelPackage, $scope.hd, $scope.channelFullName];
    $scope.channelGenres = channelGenres.join(', ');
  ...
  $dtvApi.actorInfo(17605).then(function(data) {
    setActorInfo(data);
