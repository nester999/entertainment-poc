desktop.controller('desktop.MusicModalController', function($scope, $element, $state, $stateParams, $previousState, $timeout, $dtvApi){
  //TO-DO: refactor
  ...
  $dtvApi.details($stateParams.media_type, $stateParams.id).then(function(data){
    $scope.card = data;
    // TO-DO: add hardcoded values to data

    $scope.album = 'Hungry Ghosts';
    $scope.band = 'OK GO';
    $scope.song = 'The Writing\'s On the Wall';
    $scope.lyrics = 'Listen, I know it\'s been hard. you know it\'s no different for me. We\'re less than a zero-sum game now, and baby we both know that\'s not how it\'s supposed to be.' +
                    'The writing\'s on the wall - it seems like forever since we had a good day. The writing\'s on the wall, and I...I just want to get you high tonight. I just wanna see some pleasure in your eyes.' +
                    'And I go too hot, and you go too cold and we both fall apart, and you bring your mind to rest against mine, but the mind has no say in affairs';
    var musicGenres = ['Indie/Alternative', 'Music Video', 'One-Take'];
    $scope.genres = musicGenres.join(', ');
  ...
  function setModalTabs() {
    $scope.modalTabs = {
      tabs: [{
        name: 'More From ' + $scope.album,
        value: 'moreFromAlbum',
        active: false
      }, {
        name: 'More From ' + $scope.band,
        value: 'moreFromBand',
        active: false
      }, {
