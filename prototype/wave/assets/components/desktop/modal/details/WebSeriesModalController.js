desktop.controller('desktop.WebSeriesModalController', function($scope, $element, $state, $stateParams, $previousState, $timeout, $dtvApi){
  //TO-DO: refactor
  ...
  $dtvApi.details($stateParams.media_type, $stateParams.id).then(function(data){
    $scope.card = data;
    // Hard coded
    data.webChannel = 'Nerdist';
    $scope.webChannel = data.webChannel;
  ...
  $scope.isShow = true;
  $scope.isWebSeries = true; // ($stateParams.media_type === 'web-series');
  ...
  function setModalTabs() {
    ...
    $scope.modalTabs = {
      tabs: [{
        ...
        name: 'Similar Web Series',
        value: 'similarWebSeries',
        active: false
      }, {
        name: 'More From ' + $scope.webChannel,
        value: 'moreFromWebChannel',
        active: false
      }],
      activeTab: '',
      active: false,
